...
双击 OpenCode.app 没反应，或者弹出"无法打开，因为无法验证开发者"的提示。

**原因：**

这通常发生在以下情况：
- 从非官方渠道（如第三方网站）下载的安装包
- 自行编译的开发版本
- 网络问题导致下载不完整

官方发布的桌面应用已经过 Apple 签名和公证，正常情况下不会触发此问题。如果你从官方渠道下载仍遇到拦截，可以用以下命令解除：
...
::: tip 提示
这个命令会清除应用的隔离属性（quarantine flag），告诉 macOS 你信任这个应用。只需执行一次。
:::
